...
public final class LoginService implements LoginUseCase {
  ...
  private UserModel findUserByEmail(final UserEmail email) {
    final Optional<UserModel> user = getUserByEmailPort.getByEmail(email);
    return user.orElseThrow(InvalidCredentialsException::becauseCredentialsAreInvalid);
  }
  ...
  private void validateUserCanLogin(final UserModel user) {
